...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None,defopendir = None):
        super(MainWindow, self).__init__()
        
        self.defopendir = defopendir
        
        self.setWindowTitle(__appname__)
        
        defaultSaveDir= defopendir
        ...
        if self.filePath and os.path.isdir(self.filePath):
            self.openDirDialog(dirpath=self.defopendir, silent=True)
    ...
    def openDirDialog(self, _value=False, dirpath=None, silent=False):
        if not self.mayContinue():
            return
        dirpath = self.defopendir
        defaultOpenDirPath = self.defopendir
        if self.lastOpenDir and os.path.exists(self.lastOpenDir):
    ...
    def importDirImages(self, dirpath = None):
    ...
    def currentPath(self):
        return defopendir if self.filePath else '.'
...
def get_main_app(argv=[],opendir = None):
    """
    ...
    win = MainWindow(argv[1] if len(argv) >= 2 else None,
                     ...
                     argv[3] if len(argv) >= 4 else None ,opendir)
    win.show()
    return app, win
...
#     app, _win = get_main_app(sys.argv)
    defopendir = opendir
    app, _win = get_main_app(sys.argv,defopendir)    
    return app.exec_()
